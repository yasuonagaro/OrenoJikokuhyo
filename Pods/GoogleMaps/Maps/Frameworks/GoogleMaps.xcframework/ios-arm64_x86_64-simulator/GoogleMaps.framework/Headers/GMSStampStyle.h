 ...
 *
 * @param image A `UIImage` object to use as the stamp image.
 ...
 *
 * @note Only supports transparent background. If any color is set to the `GMSStyleSpan` it will
 * only be treated as a fallback.
 *
 * @note Use of `GMSSpriteStyle` requires Metal rendering framework. See
 ...
 *
 * @note Use `GMSMapView::mapCapabilities` property to query if a `GMSMapView` supports
 * `GMSSpriteStyle`.
 */
